    ...
    class Program
    {
        static void Main()
        {
            ...
            while (true)
            {
                ...
                {
                    int startIndex = int.Parse(comand[1]), endIndex = int.Parse(comand[2]);
                    Merge(input, startIndex, endIndex);
                }
                else if (comand[0] == "divide")
                {
                    int index = int.Parse(comand[1]), partitions = int.Parse(comand[2]);
        ...
        static void Divide(List<string> input, int index, int partitions)
        {
            ...
            int partSize = partitionData.Length / partitions, remainder = partitionData.Length % partitions;
            ...
            for (int i = 0; i < partitions; i++)
            {
                ...
                {
                    tmpString += partitionData.Substring(partitionData.Length - remainder);
                }
